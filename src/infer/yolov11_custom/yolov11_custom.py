...
def infer_yolov11_custom(weight_path, data_dir):
    ...
    with open("./custom_predict.txt", "w") as file:
        for image_name in progress_bar:
            ...
            det = model(image_path, imgsz=1280, conf=0.01, iou=0.45, verbose=False)[0]
            conf_scores = det.boxes.conf.data.cpu().numpy()
            labels = det.boxes.cls.data.cpu().numpy()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Inference yolov11 sustom")
    parser.add_argument(
        "--weigth_path",
        type=str,
        default="yolov10l.pt"
